...
int my_strlen2(char *s) //using pointers another version, illustrate that argument is a local variable 
{
	int l=0;
	while(*s++ != '\0')
		l++;
	return l; //though s has move to the end of the string, it will not change the starting address of the string passed in the main(), as s is local to this function
}
...
/*	while((*d = *s) != '\0'){  // one approach to copy the string
	...
	while((*d++ = *s++));  //another way, and it is the simplest code!, the unary operator has R->L associativity
	//as d is at \0', restore it to its starting position already saved in tmp
	d = tmp;
	return d;	
}
...
/*  check the values of *d and the number of bytes it has moved from tmp 
	printf("*d =  %c, pos = %d \n", *d, (int) (d-tmp));*/
	while((*d++ = *s++ )) //another possible way, smarter!, null char is also assigned in the loop
		;
