...
int main()
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (s != SUM)
		{ // some digit is repeated in row i
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (s != SUM)
		{ // some digit is repeated in col i
			printf("wrong entries in col %d\n", i);
			return 0;
		}
	}
	// instead of writing separate code for rows, cols;
	// you can check both rows and columns simultaneously by
	// using two variables to keep track of the row sum and col sum
		...
		for (j = 0; j < 2; j++)
		{
			...
			for (int k = i * 2; k < (i * 2 + 2); k++)
				for (int l = j * 2; l < (j * 2 + 2); l++)
					s += a[k][l] - '0';
			if (s != SUM)
			{ // some digit is repeated in grid(i,j)
